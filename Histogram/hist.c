...
unsigned char **imageCropped = NULL;
...
/* for image1[ ][ ]，x_size1，y_size1 */
{
    ...
    image1 = (unsigned char *) malloc(MAX_IMAGESIZE * MAX_IMAGESIZE * sizeof(unsigned char));
...
/* Output cropped imageCropped[][], x_cropped, y_cropped */
{
    ...
    imageCropped = (unsigned char *) malloc(MAX_IMAGESIZE * MAX_IMAGESIZE * sizeof(unsigned char));
...
/* Output of image2[ ][ ], x_size2, y_size2 */
{
    ...
    image2 = (unsigned char *) malloc(MAX_IMAGESIZE * MAX_IMAGESIZE * sizeof(unsigned char));
...
/* Histogram of image1 is output into image2 */
{
    ...
    printf("Maximum frequency = %ld at level of %d\n\n", max_frequency, top);
    /* Histogram image generation */
    x_size2 = MAX_IMAGESIZE;
    y_size2 = MAX_IMAGESIZE;
    for (y = 0; y < y_size2; y++) {
        for (x = 0; x < x_size2; x++) {
            image2[y][x] = 0;
        }
    ...
    for (i = 0; i < GRAYLEVEL; i++) {
        ...
        for (j = 0; j < height; j++) {
            image2[MAX_IMAGESIZE - 1 - j][i] = MAX_BRIGHTNESS;
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    load_image_file(fName);      /* Input of image1 */
    crop_image_file(fName);      /* Crop image1 and input the data into imageCropped */
    make_histogram_image(fName); /* Histogram generation */
    save_image_file(fName);      /* Output of image2 */
    return 0;
}
